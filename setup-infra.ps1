...
function Install-Dependency {
    ...
    if (Get-Command $CommandName -ErrorAction SilentlyContinue) {
        ...
        # Reload env path to detect new executable
        $env:Path = [System.Environment]::GetEnvironmentVariable('Path','Machine') + ';' + [System.Environment]::GetEnvironmentVariable('Path','User')
        if (-not (Get-Command $CommandName -ErrorAction SilentlyContinue)) {
            Write-Error 'Failed to install dependency.'
        }
    }
}
...
Write-Host 'Setting up Kubernetes Cluster (Kind)...' -ForegroundColor Cyan
...
'@
    ...
    kind create cluster --name idp-demo --config .\kind-config.yaml
...
Write-Host '   URL: http://localhost:8080 (Username: admin)'
